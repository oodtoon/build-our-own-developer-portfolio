<script lang="ts">
  ...
  let isDeveloperSkill: boolean = false;

  const MISSING_SKILL_INDEX: number = -1;

  $: if ($developerStats.skills) {
    let foundSkillIndex = $developerStats.skills.findIndex(
      (devSkill) => devSkill.id === skill.id
    );
    isDeveloperSkill = foundSkillIndex !== MISSING_SKILL_INDEX;
  }
  ...
  function removeFromDeveloper() {
    $developerStats.skills = $developerStats.skills.filter(
      (devSkill) => devSkill.id !== skill.id
    );
